...
export default class DialogManager extends Singleton {
  /** 对话框等待弹出队列 */
  private _popWaitQueues: DialogBase[] = [];

  private dialogCloseListener: Function;

  private dialogShowListener: Function;
   ...
   */
  public showDialog(dialog: DialogBase, isPopWait: boolean = false): void {
   ...
   */
  public setDialogCloseListener(dialogCloseListener: Function){
   ...
   */
  public setDialogShowListener(dialogShowListener: Function){
   ...
   * @param name 对话框名字
   */
  public getDialog(name: string): DialogBase {
    let manager = Laya.Dialog.manager;
    if (manager.numChildren > 0) {
      for(let i=manager.numChildren-1;i>=0;i--){
        let dialog: DialogBase = manager.getChildAt(i) as DialogBase;
        if(dialog.name == name){
          return dialog;
        }
      }
    ...
    for(let i=0;i<this._popWaitQueues.length;i++){
      let dialog: DialogBase = this._popWaitQueues[i];
      if(dialog.name == name){
        return dialog;
      }
    }
  ...
  onDialogClose(dialog: DialogBase): void {
    ...
    if (this._popWaitQueues.length > 0 && manager.numChildren <= 1) {
      const dialog: DialogBase = this._popWaitQueues[0];
      dialog.popup();
  ...
  createDialogDefaultShowEffect(isFullScreen?: boolean):Laya.Handler{
    let showEffect = new Laya.Handler(
      Laya.Dialog.manager,
      function(dialog: DialogBase) {
        ...
        Laya.Tween.to(
          ...
          350,
          Laya.Ease.backOut,
          Laya.Handler.create(Laya.Dialog.manager, Laya.Dialog.manager.doOpen, [
            dialog
          ]),
          40
        );
    ...
    let closeEffect = new Laya.Handler(
      ...
      function(dialog, type) {
        ...
        Laya.Tween.to(
          ...
          280,
          null,
          Laya.Handler.create(this, this.doClose, [dialog, type])
        );
      }
    );
    return closeEffect;
