...
export default class DialogBase extends Laya.Dialog {
  ...
  private _isFullScreenExit:boolean;
  private _isEffectEnabled:boolean = true;
  private curShowEffect:Laya.Handler;
  private curShowEffectUserSet: boolean;
  private curCloseEffect:Laya.Handler;
  private curCloseEffectUserSet: boolean;
  private maskBgAlpha:number;
   ...
   */
  public setShowEffect(showEffect?:Laya.Handler){
   ...
   */
  public setCloseEffect(closeEffect?:Laya.Handler){
   ...
   */
  public setEffectEnabled(effectEnabled) {
  ...
  onOpened(param: any): void {
    ...
    if (this.loganName) {
      ...
      LoggerUtil.getInstance().warn(
        '[DialogBase] onOpened loganName error!',
  ...
  onClosed(): void {
    ...
    if (manager.numChildren > 0) {
      let nextDialog = manager.getChildAt(
        manager.numChildren - 1
      );
    ...
    if (this.loganName) {
      ...
      LoggerUtil.getInstance().warn(
        '[DialogBase] onClosed loganName error!',
        this.constructor.name
      );
    }
    super.onClosed();
