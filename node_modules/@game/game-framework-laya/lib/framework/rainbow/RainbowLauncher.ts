...
export default class RainbowLauncher extends EventDispatcher {
  private static _instance: RainbowLauncher;

  private isSupportWebp: boolean;

  private useMgcConfig: boolean;
  ...
  static getInstance(): RainbowLauncher {
    if (!this._instance) {
  ...
  private onPortmConfigSuccess() {
    if (process.env.minigame && wx.getUpdateManager) {
      ...
      if (gameVersion && minimumGameVerison) {
        if (
          VersionUtil.getInstance().compareVersion(
            gameVersion,
            minimumGameVerison
          ) < 0
        ) {
          this.backtracingOnce('updateManagerUpdateReady', this, () => {
            LoggerUtil.getInstance().info(
              '[UpdateManager] onPortmConfigSuccess onUpdateReady'
            );
            let isExecute = this.event(
              RainbowMessageType.minimumGameVerisonUpdateReady
            );
            if (!isExecute) {
              if (wx.showModal) {
                wx.showModal({
  ...
  public init(option: {
    ...
    AnalyticsUtil.getInstance().setCategory(
      process.env.compile_lxCategory ? process.env.compile_lxCategory : 'game'
    );
    ...
    LoggerUtil.getInstance().info(
      '[RainbowLauncher] Laya.Browser.width',
      Laya.Browser.width,
      'Laya.Browser.height',
      Laya.Browser.height,
      'Laya.stage.width',
    ...
    if (process.env.minigame) {
      ...
      if (process.env.webp) {
        if (this.isSupportWebp) {
          Laya.MiniAdpter.setNativeFiles(
            (<unknown>process.env.minigameNativeFileList) as string[]
          );
        }
      } else {
        Laya.MiniAdpter.setNativeFiles(
    ...
    let resourceVersion = process.env.resourceVersion as {};
    if (process.env.webp && !this.isSupportWebp) {
      Object.keys(resourceVersion).forEach((key: string) => {
        let value: string = resourceVersion[key];
    ...
    LoggerUtil.getInstance().log(
      '[RainbowLauncher] resourceVersion',
      resourceVersion
    );
    ...
    Object.keys(mergeIntoCodeAtlasJsonMap).forEach(key => {
      ...
      if (suffix == 'json') {
        ...
        LoggerUtil.getInstance().error(
          '[RainbowLauncher] mergeIntoCodeAtlasJsonMap error',
          key
        );
      }
    });
    ...
    AppBridge.getInstance().getSystemInfo({
      success: res => {
        LoggerUtil.getInstance().info(
          '[RainbowLauncher] getSystemInfo success'
        );
        if (requestConfig) {
      ...
      if (process.env.minigame) {
        if (process.env.wechat) {
          ...
          if (
            ...
            VersionUtil.getInstance().compareVersion(sdkVersion, '11.13.400') >=
            0
          ) {
            isSupport = true;
          }
        ...
        if (ua.indexOf('iPhone') > 0) {
          try {
            const version: string = ua
              .match(/iPhone OS (\S*)/)[1]
              .split('_')
              .join('.');
            LoggerUtil.getInstance().info(
              '[RainbowLauncher] ios miniprogram version',
              version
            );
            isSupport =
        ...
        if (ua.indexOf('iOS/') > 0) {
          try {
            ...
            LoggerUtil.getInstance().info(
              '[RainbowLauncher] ios web version',
              version
            );
            isSupport =
              VersionUtil.getInstance().compareVersion(version, '14.0.0') >= 0;
