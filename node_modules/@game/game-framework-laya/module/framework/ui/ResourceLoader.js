...
export default class ResourceLoader {
     ...
     * @param callback 加载回调
     */
    static load(arr, caller, callback) {
        let promiseArr = [];
        let commonArray = [];
    ...
    static loadHttpRequest(request) {
    ...
    static loadFont(fontPath) {
        return new Promise((resolve, reject) => {
            if (this.bitmapFontMap.has(fontPath)) {
                ...
                bitmapFont.loadFont(fontPath, new Laya.Handler(this, isSuccess => {
                    if (isSuccess) {
    ...
    static removeFont(fontPath) {
        let bitmapFont = this.bitmapFontMap.get(fontPath);
        if (bitmapFont) {
            bitmapFont.destroy();
            this.bitmapFontMap.delete(fontPath);
        }
    }
    static loadCommon(commonPathArray) {
        return new Promise((resolve, reject) => {
            Laya.loader.load(commonPathArray, Laya.Handler.create(this, function (isSuccess) {
                if (isSuccess) {
                    resolve({});
                }
                else {
...
ResourceLoader.bitmapFontMap = new Map();
